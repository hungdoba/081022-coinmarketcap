...
const ExchangeListCard: React.FC<Props> = ({ top }) => {
  ...
  useEffect(() => {
    if (exchangeList.value.length === 0 && exchangeList.status === "IDLE") {
      dispatch(
        fetchExchangeList({
          exchangeQueryParams: exchangeList.exchangeQueryParams,
          append: false,
        })
      );
    }
  ...
  return (
    <Box display="flex" flexWrap="wrap">
      {exchangeList.status === "LOADING" ? (
        <>
          {Array.from(
            Array(exchangeList.exchangeQueryParams.perPage).keys()
          ).map((index: number) => {
            return (
              <Box key={index} width={`${100 / cardsPerRow}%`} padding={1}>
                {index === 0 && <div ref={top} />}
                <CardSkeleton />
              </Box>
            );
          ...
          {exchangeList.value.map((exchange: Exchange, index: number) => {
            return (
              <Box key={index} width={`${100 / cardsPerRow}%`} padding={1}>
                ...
                {exchangeList.value.length === index + 1 && (
                  <div ref={lastRef} />
                )}
              </Box>
            );
