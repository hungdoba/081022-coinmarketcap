...
import { getReferralLink } from "../common/helpers/referralLink";
...
export const fetchExchangeList = createAsyncThunk(
  ...
  async (params: Params) => {
    ...
    const response = await http.request({
      ...config("coinGecko"),
      url: API.exchanges(
        params.exchangeQueryParams.page,
        params.exchangeQueryParams.perPage
      ),
      cancelToken: canceler.token,
    });

    const normalizedResponse = toCamelCase(response.data) as Exchange[];

    const exchangeListReferral = normalizedResponse.map((exchange) => {
      return getReferralLink(exchange);
    });

    return { data: exchangeListReferral, append: params.append } as {
      data: Exchange[];
      append: boolean;
    };
  }
